...
- **`outputMode: "server"`** — produce sia `dist/psicologo/browser/` (statico) sia `dist/psicologo/server/server.mjs` (Node)
...
`src/assets/mock/*.json` — file statici importati via `HttpClient` o `fetch` lato client, oppure via direct `import` lato server (SSR). Vedi pagina [Mock Data](/mock-data) per la struttura specifica di Psicologo.
